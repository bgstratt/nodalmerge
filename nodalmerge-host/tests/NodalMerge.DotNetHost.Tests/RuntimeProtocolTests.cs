...
public class RuntimeProtocolTests
{
    ...
    [Fact]
    public void Query_register_requires_query_admin_capability()
    {
        var mapper = new RuntimeProtocolMapper();
        var state = new RuntimeConnectionState(1)
        {
        ...
        var result = mapper.MapIncomingMessageToCommandJsons(
            "{\"type\":\"query.register\",\"query_spec_id\":\"q.rooms\",\"version\":\"v1\",\"descriptor\":{}}",
            state
        );

        Assert.False(result.IsSuccess);
        Assert.Equal("reject.control_plane_forbidden: command=query.register requires=query.admin", result.Error);
    }
    ...
    [Fact]
    public void Projection_invalidate_requires_query_admin_capability()
    {
        var mapper = new RuntimeProtocolMapper();
        var state = new RuntimeConnectionState(1)
        {
        ...
        Assert.False(result.IsSuccess);
        Assert.Equal("reject.control_plane_forbidden: command=projection.invalidate requires=query.admin", result.Error);
    }

    [Fact]
    public void Archive_describe_maps_to_host_command_when_archive_read_capability_present()
    {
        var mapper = new RuntimeProtocolMapper();
        var state = new RuntimeConnectionState(1)
        {
        ...
        state.SessionCapabilities.Add("archive.read");
        ...
        Assert.True(result.IsSuccess);
        Assert.Single(result.CommandJsons);
        Assert.Contains("DescribeArchive", result.CommandJsons[0]);
        Assert.Contains("\"archive_ref\":\"s3://bucket/room-a.nmar\"", result.CommandJsons[0]);
    }

    [Fact]
    public void Archive_validate_requires_archive_admin_capability()
    {
        var mapper = new RuntimeProtocolMapper();
        var state = new RuntimeConnectionState(1)
        {
        ...
        var result = mapper.MapIncomingMessageToCommandJsons(
            "{\"type\":\"archive.validate\",\"archive_ref\":\"s3://bucket/room-a.nmar\",\"mode\":\"metadata_only\"}",
            state
        );

        Assert.False(result.IsSuccess);
        Assert.Equal("reject.control_plane_forbidden: command=archive.validate requires=archive.admin", result.Error);
    }

    [Fact]
    public void Archive_import_maps_to_host_command_when_archive_admin_capability_present()
    {
        var mapper = new RuntimeProtocolMapper();
        var state = new RuntimeConnectionState(1)
        {
        ...
        var result = mapper.MapIncomingMessageToCommandJsons(
            "{\"type\":\"archive.import\",\"archive_ref\":\"s3://bucket/room-a.nmar\",\"import_mode\":\"full_clone\",\"expected_checkpoint\":{\"canonical_hash\":\"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa\"}}",
            state
        );

        Assert.True(result.IsSuccess);
        Assert.Single(result.CommandJsons);
        Assert.Contains("ImportArchive", result.CommandJsons[0]);
        Assert.Contains("\"archive_ref\":\"s3://bucket/room-a.nmar\"", result.CommandJsons[0]);
        Assert.Contains("\"import_mode\":\"full_clone\"", result.CommandJsons[0]);
        Assert.Contains("\"expected_checkpoint\":{\"canonical_hash\":\"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa\"}", result.CommandJsons[0]);
    }
    ...
    [Fact]
    public void Event_mapper_converts_query_projection_events_to_runtime_messages()
    {
        var mapper = new RuntimeProtocolMapper();
        var eventsJson =
            "[" +
            "{\"QuerySpecRegistered\":{\"room_id\":\"room-a\",\"query_spec_id\":\"q.rooms\",\"version\":\"v1\",\"canonical_hash\":\"abc\",\"accepted\":true}}," +
            "{\"QuerySpecRejected\":{\"room_id\":\"room-a\",\"query_spec_id\":\"q.rooms\",\"version\":\"v2\",\"reason_class\":\"reject.query_unsupported_version\",\"reason_message\":\"unsupported\"}}," +
            "{\"ProjectionBuildCompleted\":{\"room_id\":\"room-a\",\"projection_id\":\"p.rooms\",\"checkpoint\":{\"selector\":\"seq\",\"canonical_seq\":1,\"canonical_hash\":\"abc\"},\"digest\":\"dig-1\"}}," +
            "{\"ProjectionBuildRejected\":{\"room_id\":\"room-a\",\"projection_id\":\"p.rooms\",\"reason_class\":\"reject.checkpoint_selector_invalid\",\"reason_message\":\"selector hash requires canonical_hash in 64-char hex format\"}}," +
            "{\"ProjectionReadResult\":{\"room_id\":\"room-a\",\"projection_id\":\"p.rooms\",\"checkpoint\":{\"selector\":\"seq\",\"canonical_seq\":1,\"canonical_hash\":\"abc\"},\"rows\":[{\"k\":\"world/a\",\"v\":\"1\"}],\"digest\":\"dig-1\",\"next_page_token\":\"tok-2\"}}," +
            "{\"ProjectionInvalidated\":{\"room_id\":\"room-a\",\"projection_id\":\"p.rooms\",\"reason\":\"manual\",\"invalidated_at_hlc\":123}}," +
            "{\"ProjectionListResult\":{\"room_id\":\"room-a\",\"query_spec_id\":\"q.rooms\",\"items\":[{\"projection_id\":\"p.rooms\"}],\"cursor\":\"cur-1\"}}" +
            "]";

        var result = mapper.MapEventsJsonToOutboundMessages(eventsJson);

        Assert.True(result.IsSuccess);
        Assert.Equal(7, result.OutboundMessages.Count);
        Assert.Contains("\"type\":\"query.registered\"", result.OutboundMessages[0]);
        Assert.Contains("\"type\":\"query.register.rejected\"", result.OutboundMessages[1]);
        Assert.Contains("\"type\":\"projection.build.completed\"", result.OutboundMessages[2]);
        Assert.Contains("\"checkpoint\":{\"selector\":\"seq\",\"canonical_seq\":1,\"canonical_hash\":\"abc\"}", result.OutboundMessages[2]);
        Assert.Contains("\"digest\":\"dig-1\"", result.OutboundMessages[2]);
        Assert.Contains("\"type\":\"projection.build.rejected\"", result.OutboundMessages[3]);
        Assert.Contains("\"reason_class\":\"reject.checkpoint_selector_invalid\"", result.OutboundMessages[3]);
        Assert.Contains("\"reason_message\":\"selector hash requires canonical_hash in 64-char hex format\"", result.OutboundMessages[3]);
        Assert.Contains("\"type\":\"projection.read.result\"", result.OutboundMessages[4]);
        Assert.Contains("\"checkpoint\":{\"selector\":\"seq\",\"canonical_seq\":1,\"canonical_hash\":\"abc\"}", result.OutboundMessages[4]);
        Assert.Contains("\"digest\":\"dig-1\"", result.OutboundMessages[4]);
        Assert.Contains("\"type\":\"projection.invalidated\"", result.OutboundMessages[5]);
        Assert.Contains("\"type\":\"projection.list.result\"", result.OutboundMessages[6]);
    }

    [Fact]
    public void Event_mapper_converts_archive_events_to_runtime_messages()
    {
        var mapper = new RuntimeProtocolMapper();
        var eventsJson =
            "[" +
            "{\"ArchiveDescribed\":{\"room_id\":\"room-a\",\"archive_ref\":\"s3://bucket/room-a.nmar\",\"manifest_id\":\"m.stub.0001\",\"format_version\":\"1\",\"archive_kind\":\"full_clone\",\"checkpoint\":{\"frontier\":[\"seq:0\"],\"canonical_hash\":\"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa\"}}}," +
            "{\"ArchiveValidated\":{\"room_id\":\"room-a\",\"archive_ref\":\"s3://bucket/room-a.nmar\",\"accepted\":true,\"mode\":\"metadata_only\",\"checks\":[\"manifest\"]}}," +
            "{\"ArchiveValidationRejected\":{\"room_id\":\"room-a\",\"archive_ref\":\"s3://bucket/bad.nmar\",\"reason_class\":\"reject.archive_manifest_invalid\",\"reason_message\":\"manifest missing required field\"}}," +
            "{\"ArchiveImported\":{\"room_id\":\"room-a\",\"archive_ref\":\"s3://bucket/room-a.nmar\",\"canonical_hash\":\"bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb\",\"checkpoint\":{\"frontier\":[\"seq:0\"],\"canonical_hash\":\"bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb\"},\"imported_nodes\":10,\"imported_blobs\":2}}," +
            "{\"ArchiveImportRejected\":{\"room_id\":\"room-a\",\"archive_ref\":\"s3://bucket/bad.nmar\",\"reason_class\":\"reject.archive_digest_mismatch\",\"reason_message\":\"payload digest mismatch\"}}" +
            "]";

        var result = mapper.MapEventsJsonToOutboundMessages(eventsJson);

        Assert.True(result.IsSuccess);
        Assert.Equal(5, result.OutboundMessages.Count);
        Assert.Contains("\"type\":\"archive.describe.result\"", result.OutboundMessages[0]);
        Assert.Contains("\"type\":\"archive.validate.result\"", result.OutboundMessages[1]);
        Assert.Contains("\"type\":\"archive.validate.rejected\"", result.OutboundMessages[2]);
        Assert.Contains("\"reason_class\":\"reject.archive_manifest_invalid\"", result.OutboundMessages[2]);
        Assert.Contains("\"type\":\"archive.import.completed\"", result.OutboundMessages[3]);
