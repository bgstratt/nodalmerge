        ...
        return new ConfigurationBuilder()
            .AddInMemoryCollection(
                new Dictionary<string, string?>
                {
                    ...
                    ["ActiveSync:Storage:FileBlobs:RootPath"] = Path.Combine(Path.GetDirectoryName(dbPath)!, "blobs")
                }
            )
            .Build();
    }
            ...
            configureConfiguration: cfg =>
            {
                cfg.AddInMemoryCollection(
                    new Dictionary<string, string?>
                    {
                        ...
                        ["ActiveSync:Runtime:Dag:Compaction:Enabled"] = "true",
                        ["ActiveSync:Runtime:Dag:Compaction:MinEligibleNodes"] = "4",
                        ["ActiveSync:Runtime:Dag:Compaction:EnablePruning"] = enablePruning ? "true" : "false"
                    }
                );
            }
        );
