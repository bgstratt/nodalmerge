...
## Text Runtime Migration Plan (Incremental Projection)

Goal: Keep existing RGA convergence semantics while replacing full replay-per-resolve with an incrementally maintained text projection per key.
...
| `bridge` (WASM Rust API surface) | ⚠️ likely small | ⚠️ optional (debug toggles) | ✅ required (range APIs) | ⚠️ optional |
| JS SDK / web bindings (`pkg`, wrapper helpers) | ⚠️ likely small | ⚠️ optional | ✅ required | ⚠️ optional |
| host integrations (`dotnet-host`, `server`) | ❌ none expected | ❌ none expected | ⚠️ only if adopting new range APIs immediately | ❌ none expected |

Interpretation:
- Most implementation work is in `core`.
- Host/server can stay mostly untouched through Phase 1/1.5.
- Bridge/SDK updates are mostly API ergonomics and optional feature toggles until range ops land.
...
- [ ] Route `resolve_text` / `resolve_text_seq` through projection when enabled.
...
- [ ] No replay/convergence regressions in concurrent insert/delete scenarios.

### Phase 1.5 — Safety Fallback + Parity Checks

Status: [ ] not started

Execution checklist:
- [ ] Add feature flag for projection read path (example: `text_projection`).
- [ ] Add optional parity-check mode that compares projection results against legacy resolver for sampled keys/calls.
...
### Phase 2 — Range Ops While Preserving Char Compatibility
...
### Phase 3 — Counted Position Index (`O(log n)` Mapping)

Status: [ ] not started

Execution checklist:
- [ ] Introduce counted index structure for visible length accounting (tree/rope/B-tree variant).
- [ ] Implement `offset -> anchor` and `anchor -> offset` APIs using subtree counts.
...
- [ ] Add projection-specific unit tests:
  ...
  - [ ] multi-key isolation
- [ ] Add property-style parity tests (projection output equals legacy output for randomized traces).
- [ ] Extend `text_trace_rustcode` benchmarking with projection on/off variants.
- [ ] Track perf deltas in repo memory notes after each phase.
...
- [ ] Snapshot wire format changes are out of scope unless needed for future projection persistence.
...
- Bridge/SDK updates are expected but can be staged and mostly backward compatible.
