...
|---|---|---|---|---|---|---|---|---|---|---|
| RNM-001 | Rust crates | Cargo.toml + */Cargo.toml | ArtifactName | activesync-core | nodalmerge-core | Transitional aliases or dual-publish for external crates | Wave R through next full release | Rust Platform Stream | InProgress | Completed dedicated crate-ID migration wave: canonical implementation crates now use `nodalmerge-*` package IDs, compatibility wrappers now provide `activesync-*` package IDs, and `Cargo.lock` was regenerated with workspace validation (`cargo check --workspace`, `cargo test --workspace --no-run -j 1`) on 2026-05-25 |
...
| RNM-014 | Bench/run scripts | *.ps1, benchmarks/**/*.ps1 | ScriptRef | cargo run -p activesync-server | cargo run -p nodalmerge-server --bin nodalmerge-server | Legacy command compatibility wrappers | Wave R through next full release | Performance Tooling Stream | InProgress | Root + benchmark launch scripts are nodalmerge-first for container/env/database naming, dotnet host project resolution, and crate IDs (`cargo run -p nodalmerge-*`) while retaining explicit legacy compatibility notes where required (2026-05-25) |
| RNM-015 | CI/release pipelines | workflow/release scripts | ScriptRef | activesync artifact ids | nodalmerge artifact ids | Dual publish until switch complete | Wave R through next full release | Build/Release Stream | InProgress | Workflow and script cargo invocations are now nodalmerge-first (`cargo build/check/test -p nodalmerge-*`), while explicit legacy compatibility package publishing remains in place for migration window (2026-05-25) |
