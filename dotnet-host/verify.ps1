...
try {
    ...
    elseif ([string]::IsNullOrWhiteSpace($env:ACTIVESYNC_HOST_FFI_DLL)) {
        ...
        if ($resolvedFfiDll) {
            ...
            Write-Host "Using ACTIVESYNC_HOST_FFI_DLL=$resolvedFfiDll"
        }
        else {
            Write-Host "Warning: ACTIVESYNC_HOST_FFI_DLL is not set and no local host-ffi DLL was found."
    ...
    $runtimeArgs = @(
        ...
        "--ActiveSync:Storage:S3Delegated:GetPath=/v1/blobs/presign-get",
        "--ActiveSync:Storage:S3Delegated:TimeoutSeconds=5",
        "--ActiveSync:Storage:S3Delegated:MaxRetries=2",
        "--ActiveSync:Storage:S3Delegated:CircuitBreakerFailureThreshold=3",
        "--ActiveSync:Storage:S3Delegated:CircuitBreakerOpenSeconds=30"
    )
