...
| Blob storage layout parity unaudited (Rust file/S3 layout vs .NET FileBlobStoreProvider/S3Delegated) | **Audited 2026-07-06, convergence planned** — audit found fully incompatible file layouts (Rust `blobs/<room>/<hex>` vs .NET `<shard>/<hex>.blob`), divergent tombstones, and mismatched delegate payloads; decision: converge on global CAS `blake3/<hex>` (matches live SpeechSlate S3 data), breaking change at 0.2.0 with auto-migration markers. Plan: `docs/BLOB_LAYOUT_CONVERGENCE_PLAN.md` |
